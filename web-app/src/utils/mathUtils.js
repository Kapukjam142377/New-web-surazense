...
export function baselineCorrection(readFREQ, dataMag, order = 8) {
  if (!readFREQ || !dataMag || readFREQ.length < order + 1) {
    throw new Error(`Insufficient data points (${readFREQ?.length || 0}) to perform polynomial regression of order ${order}.`);
  }
  ...
  try {
    ...
    throw new Error(`Polynomial regression failed: ${error.message}. The input signal might be flat or unstable.`);
  }
}
...
export function evaluateSpline(spline, xVal) {
  ...
  if (i >= n - 1) i = n - 2;
  const t = xVal - i;
  return spline.a[i] + spline.b[i] * t + spline.c[i] * t * t + spline.d[i] * t * t * t;
}
